...
#cgo linux LDFLAGS: -L. -lluajit_linux -lm -ldl
#cgo darwin LDFLAGS: -L. -llua_osx
...
func (L *State) DumpStack(msg string) {
	i := 0
	top := L.GetTop()
	fmt.Printf("----------dumpstack BEGIN----------%s\n", msg)
	fmt.Printf("%-20s\t%-20s\t%-20s\t%s\n", "Stack Index", "Field Name", "Type", "Value")
	for i = 1; i <= top; i++ {
		...
		luaType := L.Type(i)
		fmt.Printf("%-20d\t%-20s\t%-20s\t", i, strField, L.Typename(int(luaType)))
		switch luaType {
		case LUA_TNUMBER:
			fmt.Printf("%v\n", L.ToNumber(i))
		case LUA_TSTRING:
