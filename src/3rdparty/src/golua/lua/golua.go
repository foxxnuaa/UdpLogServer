...
/*
#cgo CFLAGS: -Ilua
#cgo windows LDFLAGS: -L. -llua_windows
...
func (this *State) GetCState() *C.lua_State {
	return this.s
}
...
func go_panic_msghandler(gostateindex uintptr, z *C.char) {
	...
	luaError := LuaError{LUA_ERRERR, s, L.StackTrace()}

	//kLog.Errorln("go_panic_msghandler: code=", luaError.code, " message=", luaError.message)

	kLog.Errorln("go_panic_msghandler: ", luaError)
	/*
		    local tData = {}
				tData.exception_info = exception_info
				tData.uid = tostring(USER_UID)
				tData.server_id = tostring(SERVER_ID)
				tData.type = "1"
				http.PostData(G_LUA_ERROR_URL,cjson.encode(tData))
	*/
	strErrorData := ""

	strErrorData += "{\"type\":\"1\",\"uid\":\"0\",\"server_id\":\"0\",\"exception_info\":" + "\"" + fmt.Sprintf("%v", luaError) + "\"" + "}"
	http.Post("http://log.czgame.net:6010/log/log.php", "application/octet-stream", strings.NewReader(strErrorData))
	//kLog.Errorln("strErrorData: ", strErrorData)

	// 这里不调用panic，这样异常的时候不会导致进程崩溃。
